...
module.exports = function (pdict) {
	...
	*/
	...
	switch (element) {
		...
		case 'radio':
            var optionCount = Object.keys(pdict.formfield.options).length;

			Object.keys(pdict.formfield.options).forEach(function (optionKey) {
				var option = pdict.formfield.options[optionKey];
				var value = option.value;
				var checked = '';
                var classes = "input-radio";
				if (option.checked) {
					checked = 'checked="checked"';
				}
				...
				options.push(
                    '<input class="' + classes + '"' + fieldClass + ' type="radio"' + checked + ' id="' + id + '" name="' + name + '" value="' + value + '" ' + attributes + '/>' +
                    '<span class="label">' + Resource.msg(option.label, 'forms', null) + '</span>'
                );
			});
			input += options.join('');
			break;
