...
(function (root, factory) {
    if (typeof exports === 'object') {
        ...
        // Global Variables
        root.ba.customFormElements = factory(root.jQuery, root.ba);
    }
    ...
    var customFormElements = {
        ...
        getLabel: function(el, type) {
            var elementLabel, input = $(el);

            if(input.parent().siblings('label, span.label').length && input.hasClass('multiple')) {
                return this.wrapRadioInputs(input);
            }

            if(input.siblings('label, span.label').length){
                elementLabel = input.siblings('label, span.label').first();
            } else if(input.parent().siblings('label, span.label').length && !input.hasClass('multiple')) {
        ...
        wrapRadioInputs: function(el) {
            var labelType = this.getLabelType(el);

            if(!el.parent('.field').length){
                var labelText = el.next('span').remove().text().trim();

                if (labelType === 'label') {
                    el.wrap('<div class="field"></div>');
                    el.after('<label for="' + el.attr('id') + '">' + labelText + '</label>');
                    return el.siblings('label');
                } else {
                    el.wrap('<div class="field field-inline"></div>');
                    el.after('<span class="label radio-label">' +  labelText + '</span>')
                    return el.siblings('span');
                }
        ...
        buildCustomInput: function(type, elements) {
            ...
            if(inputElements.length > 0) {
                $.each(inputElements, function(idx, el){
                    var label = self.getLabel(el, type),
        ...
        customRadios: function() {
            var radioElements =  $('input[type="radio"].input-radio'),
                self = this;
        ...
        updateRadioGroup: function(group, labelType){
            $('input[name="' + group + '"]').each(function(idx, el){
                ...
                $(el).parents('.form-row').find(labelType).removeClass('checked');
            });
        },

        // Build out to Create Custom Checkboxes and Assign Applicable Observers
        customCheckboxes: function(){
            var self = this, checkboxElements = $('input[type="checkbox"].input-checkbox');
